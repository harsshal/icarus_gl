    ...
    def calculate_indicators(self):
        ...
        for day in df['first_candle'].unique():
            ...
            if len(day_df) > 0:
                ...
                # Close position when midpoints of last 4 candles are approximately horizontal
                if len(day_df) >= 4 and position_opened and not position_closed:
                    mean_average = day_df['average'].rolling(window=4).mean()
                    std_average = day_df['average'].rolling(window=4).std()
                    if std_average.iloc[-1] < 0.1 * mean_average.iloc[-1]:
                        df.at[day_df.index[-1], 'position'] = 0  # Close position
                        position_closed = True
                        print(f"Closed position at index {day_df.index[-1]}")
